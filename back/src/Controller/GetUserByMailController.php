...
class GetUserByMailController extends AbstractController
{
    ...
    public function getCommentsByTrajetsId(Request $request,EntityManagerInterface $em, UserTrajetRepository $UserTrajetRepo, TrajetRepository $TrajetRepo): JsonResponse
    {
        $content = $request->getContent();
        $jsonParse = json_decode($content, true);
        try {
    ...
    public function deleteReservation(int $idTrajet, $idUser, EntityManagerInterface $em, UserTrajetRepository $UserTrajetRepo): JsonResponse
    {
        try {
