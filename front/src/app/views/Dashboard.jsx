...
import { faArrowRight, faVolumeControlPhone } from "@fortawesome/free-solid-svg-icons";
    ...
    const fetchTrajets = async () => {
      try {
        if (currentUser.id) {
          ...
          const resultCar = await axios.get(URL_BACK + '/get/cars/' + currentUser.id);
          ...
          const resultsComments = await axios.get(URL_BACK + '/get/comments/' + currentUser.id);
    ...
    <div className="containerCard flex flex-col h-[84vh] bg-Teal items-center ">
      ...
      <div className="w-9/12 cardInfos flex box-border border-8 bg-Moonstone text-center items-center rounded-lg">
        <div className="flex flex-col ml-20">
          <img
            className="imgProfil h-50 w-60 rounded-full object-cover"
            src={currentUser.imgProfil ? currentUser.imgProfil : '/src/app/assets/img/avatar.png'}
            alt=""
          />
          <div className="mt-10">
                ...
                <div className="mb-8 w-full flex flex-row overflow-x-scroll whitespace-nowrap scrollbar-thumb-Teal scrollbar-track-[#04adbf] scrollbar-thin">
                  {images.map((img) => (
                    <div className="imgContainer ml-5">
                      <img className="photo w-24 max-h-[60px] object-contain inline-block" src={img} />
                    </div>
                  ))}
                </div>
                <div className="mb-8">
        ...
        <div className="containerCardInfo flex justify-center items-center px-12 bg-Teal w-6/12 h-5/6 mr-20 mt-20 ml-20 rounded-xl mb-20">
          <div className="histor">
            ...
            <div>
              <p className="text-center text-white mt-12 mb-8 text-2xl font-semibold">Evaluations :</p>
